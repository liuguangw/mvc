...
class MimeHelper
{
    ...
    private static function loadMimeMap()
    {
        $mapData = [];
        foreach (self::$mimeOrgData as $mimeType => $extensions) {
            if (strpos($extensions, ' ')) {
                $extensionArr = explode(' ', $extensions);
                foreach ($extensionArr as $ext) {
                    $mapData[$ext] = $mimeType;
                }
            } else {
                $mapData[$extensions] = $mimeType;
            }
        }
        self::$mimeOrgData = null;
        self::$mimeMap = $mapData;
    }
